...
bool EspTcp::Connect(const std::string& host, int port) {
    ...
    xTaskCreate([](void* arg) {
        ...
        vTaskDelete(NULL);
    }, "tcp_receive", 8192, this, 1, &receive_task_handle_);  // Increased from 4096 to fix stack overflow
    return true;
}
...
void EspTcp::ReceiveTask() {
    ...
    while (connected_) {
        ...
        if (ret <= 0) {
            if (ret < 0) {
                ESP_LOGE(TAG, "TCP receive failed: host=%s:%d ret=%d errno=%d (%s)", 
                    connect_host_.c_str(), connect_port_, ret, errno, strerror(errno));
            } else {
                ESP_LOGW(TAG, "TCP receive returned 0 (peer closed): host=%s:%d", 
