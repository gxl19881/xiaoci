...
def setup_logging():
    ...
    if not _logger_initialized:
        ...
        # 额外输出一份“完整控制台格式”的日志，带 UTF-8 BOM，方便在 Windows PowerShell 5.1 下直接查看不乱码
        # 说明：PowerShell 5.1 对无 BOM 的 UTF-8 文本默认按本地代码页解码，导致中文显示为乱码
        # 这里使用 utf-8-sig 编码（含 BOM），以便 Get-Content 能自动识别为 UTF-8
        full_log_file = log_config.get("full_log_file", "full_server.log")
        full_log_path = os.path.join(log_dir, full_log_file)
        logger.add(
            full_log_path,
            format=log_format,  # 与控制台一致的简洁格式
            level=log_level,
            filter=formatter,
            rotation="20 MB",  # 全量日志允许稍大
            retention="30 days",
            compression=None,
            encoding="utf-8-sig",  # 关键：带 BOM，避免 PowerShell 5.1 乱码
            enqueue=True,
            backtrace=True,
            diagnose=True,
        )
