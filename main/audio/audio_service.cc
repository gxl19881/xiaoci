...
void AudioService::Start() {
    ...
    xTaskCreatePinnedToCore([](void* arg) {
        ...
        vTaskDelete(NULL);
    }, "audio_input", 8192, this, 8, &audio_input_task_handle_, 1);
...
void AudioService::OpusCodecTask() {
    while (true) {
        ...
        /* Encode the audio to send queue (or pass-through as PCM) */
        if (!audio_encode_queue_.empty() && audio_send_queue_.size() < MAX_SEND_PACKETS_IN_QUEUE) {
            ...
            if (send_pcm_) {
                // 直接发送PCM原始字节（LE 16-bit），与hello中的audio_params.format=pcm一致
                auto &pcm = task->pcm;
...
void AudioService::SetNoiseReductionMode(int mode) {
#if CONFIG_USE_AUDIO_PROCESSOR
    auto afe_processor = static_cast<AfeAudioProcessor*>(audio_processor_.get());
    if (afe_processor) {
        afe_processor->SetNoiseReductionMode(static_cast<NoiseReductionMode>(mode));
        ESP_LOGI(TAG, "Noise reduction mode set to: %d", mode);
    }
#endif
}

int AudioService::GetNoiseReductionMode() const {
#if CONFIG_USE_AUDIO_PROCESSOR
    auto afe_processor = static_cast<AfeAudioProcessor*>(audio_processor_.get());
    if (afe_processor) {
        return static_cast<int>(afe_processor->GetNoiseReductionMode());
    }
#endif
    return 1; // 默认中度降噪
}

void AudioService::SetAdaptiveNoiseReduction(bool enable) {
#if CONFIG_USE_AUDIO_PROCESSOR
    auto afe_processor = static_cast<AfeAudioProcessor*>(audio_processor_.get());
    if (afe_processor) {
        afe_processor->SetAdaptiveNoiseReduction(enable);
        ESP_LOGI(TAG, "Adaptive noise reduction %s", enable ? "enabled" : "disabled");
    }
#endif
}

bool AudioService::IsAdaptiveNoiseReductionEnabled() const {
#if CONFIG_USE_AUDIO_PROCESSOR
    auto afe_processor = static_cast<AfeAudioProcessor*>(audio_processor_.get());
    if (afe_processor) {
        return afe_processor->IsAdaptiveNoiseReductionEnabled();
    }
#endif
    return false;
}