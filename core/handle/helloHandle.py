...
async def handleHelloMessage(conn, msg_json):
    ...
    # 优先从hello消息体中提取学号
    try:
        sid = (msg_json.get("student_id") or "").strip()
        if sid:
            conn.student_id = sid
            conn.logger.bind(tag=TAG).info(f"提取到学号(来自Hello): {conn.student_id}")
            try:
                add_sid(conn.student_id)
            except Exception:
                pass
        else:
            conn.logger.bind(tag=TAG).info("Hello消息未携带学号(student_id)")
    except Exception as _e:
        conn.logger.bind(tag=TAG).warning(f"解析Hello中student_id异常: {_e}")
    ...
    if audio_params:
        ...
        # 解析端序（可选字段：endian 或 pcm_endian），值期望 'le'/'be'
        try:
            endian = (audio_params.get("endian") or audio_params.get("pcm_endian") or "").strip().lower()
            if endian in ("le", "be"):
                conn.pcm_endian = endian
                conn.logger.bind(tag=TAG).info(f"客户端PCM端序: {conn.pcm_endian}")
        except Exception as _e:
            conn.logger.bind(tag=TAG).warning(f"解析客户端PCM端序失败: {_e}")

        # 回传已解析参数（便于调试）
        ap = dict(audio_params)
        ap["endian"] = getattr(conn, "pcm_endian", "le")
        conn.welcome_msg["audio_params"] = ap
