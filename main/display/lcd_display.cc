...
void LcdDisplay::SetupUI() {
    ...
    // 创建自动滚动恢复定时器（用于历史浏览一段时间后恢复到底部自动滚动）
...
void LcdDisplay::SetChatMessage(const char* role, const char* content) {
    ESP_LOGI(TAG, "SetChatMessage: MAX_MESSAGES=%d", MAX_MESSAGES);
    DisplayLockGuard lock(this);
    if (content_ == nullptr) {
        return;
    }
    ...
    if (strcmp(role, "system") == 0 && child_count > 0) {
        // [Optimization] 针对 "正在生成图片" 类进度更新的特殊处理
        // 如果新消息包含 "正在生成" 或 "绘图"，且上一条消息也是这类消息，则强制原地更新
        bool is_progress_update = (strstr(content, "正在生成") != nullptr || strstr(content, "绘图") != nullptr);
        ...
        if (last_container != nullptr && lv_obj_get_child_cnt(last_container) > 0) {
            ...
            if (last_bubble != nullptr) {
                // 检查气泡类型是否为系统消息 (更加鲁棒的检查：UserData 或 背景色 或 文本内容)
                ...
                // [Optimization] 如果 UserData 判定失败，且当前是进度类消息，检查上一条消息是否也是进度类消息
                if (!is_system_msg && is_progress_update) {
                    lv_obj_t* prev_label = lv_obj_get_child(last_bubble, 0);
                    if (prev_label) {
                        const char* prev_txt = lv_label_get_text(prev_label);
                        // 如果上一条也是包含关键字符的消息，视为同一类系统消息
                        // 支持 "图片正在生成中" 或 "绘图"
                        if (prev_txt && (strstr(prev_txt, "正在生成") != nullptr || strstr(prev_txt, "绘图") != nullptr)) {
                            is_system_msg = true;
                            // 补救：如果之前没标记上 system，现在标记上
                            lv_obj_set_user_data(last_bubble, (void*)"system");
                        }
                    }
                }

                if (!is_system_msg) {
                    // Fallback: 如果 UserData 失效（配置原因），检查背景颜色
                    lv_color_t bg = lv_obj_get_style_bg_color(last_bubble, 0);
                    if (lv_color_eq(bg, current_theme_.system_bubble)) {
                        is_system_msg = true;
                    }
                }

                // 如果是进度更新，即使上一条不是系统消息（理论上不应发生，但为了保险），也尝试找上一条
                // 但为了逻辑严谨，我们只在上一条确实是 system 消息时才折叠
                if (is_system_msg) {
                    // 如果最后一个消息也是系统消息，则直接更新其文本，避免刷屏滚动
                    lv_obj_t* last_label = lv_obj_get_child(last_bubble, 0);
                    if (last_label != nullptr) {
                        // [Optimization] 避免不必要的重绘：如果文本完全一样，直接返回
                        const char* current_text = lv_label_get_text(last_label);
                        if (current_text && strcmp(current_text, content) == 0) {
                            return;
                        }
                        ...
                        // 重新计算并更新宽度逻辑
                        // 计算文本实际宽度
                        lv_coord_t new_text_width = lv_txt_get_width(content, strlen(content), fonts_.text_font, 0);
                        lv_coord_t max_width = LV_HOR_RES * 85 / 100 - 16;
                        lv_coord_t min_width = 20;
                        if (new_text_width < min_width) new_text_width = min_width;
                        // 气泡宽度取 文本与最大限制的较小值
                        lv_coord_t new_bubble_width = (new_text_width < max_width) ? new_text_width : max_width;
                        
                        // 强制更新宽度
                        lv_obj_set_width(last_label, new_bubble_width);
                        lv_obj_set_width(last_bubble, new_bubble_width);
                        
                        // 确保标签也是 Long Wrap 模式
                        lv_label_set_long_mode(last_label, LV_LABEL_LONG_WRAP);
    ...
    if (strcmp(role, "user") == 0) {
        ...
        // Auto-scroll to this container（若未暂停自动滚动）
        ...
        // Auto-scroll to the message bubble（若未暂停自动滚动）
        if (!autoscroll_paused_) {
...
// 翻页/历史浏览：每次按页滚动 70% 高度，并在若干秒内暂停自动滚动
void LcdDisplay::PauseAutoScrollMs(int ms) {
...
void LcdDisplay::ScrollToLatest() {
    DisplayLockGuard lock(this);
    if (!content_) return;
    uint32_t child_count = lv_obj_get_child_cnt(content_);
    if (child_count > 0) {
