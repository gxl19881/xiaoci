...
//         char buffer[1024];
//         while (1) {
//             int read_len = esp_http_client_read(client_, buffer, sizeof(buffer) - 1);
//             if (read_len <= 0) break;
//             buffer[read_len] = 0;  // null-terminate
//             total_read += read_len;
//         }
//         if (total_read > 0) {
//             return std::string(buffer, total_read);
//         }
//     } 
//     return {};
// }
...
std::string HttpClient::ReadAll() {
    ...
    // 根据 content_length_ 处理 - 融入 EspHttp 的策略
    if (content_length_ > 0) {
        // 已知长度情况 - 采用 EspHttp 的预分配策略
        std::string result(content_length_, '\0');
        size_t total_read = 0;
        
        // 适配 HttpClient 的 body_chunks_ 数据结构
        for (const auto& chunk : body_chunks_) {
            if (total_read + chunk.data.size() <= static_cast<size_t>(content_length_)) {
                std::memcpy(&result[total_read], chunk.data.data(), chunk.data.size());
                total_read += chunk.data.size();
            } else {
                // 防止超出预分配空间
                size_t remaining = static_cast<size_t>(content_length_) - total_read;
                if (remaining > 0) {
                    std::memcpy(&result[total_read], chunk.data.data(), remaining);
