...
private:
    ...
    // Execute a tool call with a configurable stack size and optional timeout (milliseconds)
    void DoToolCall(int id, const std::string& tool_name, const cJSON* tool_arguments, int stack_size, int timeout_ms = 35000);

    std::vector<McpTool*> tools_;
    std::thread tool_call_thread_;
    std::atomic<bool> tool_running_{false};
    // 强制复位运行状态（慎用，仅用于卡死恢复）
    void ForceResetRunningState() { tool_running_.store(false); }
};
