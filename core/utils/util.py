...
def check_vad_update(before_config, new_config):
    ...
    # 获取选中的模块名
    current_vad_module = before_config.get("selected_module", {}).get("VAD")
    new_vad_module = new_config.get("selected_module", {}).get("VAD")
    
    # 如果模块名变了，肯定要更新
    if current_vad_module != new_vad_module:
        return True
        
    # 获取模块的具体配置
    current_config = before_config.get("VAD", {}).get(current_vad_module, {})
    new_config_val = new_config.get("VAD", {}).get(new_vad_module, {})
    
    # 比较配置字典是否相等（包含参数变化）
    return current_config != new_config_val
...
def check_asr_update(before_config, new_config):
    ...
    # 获取选中的模块名
    current_asr_module = before_config.get("selected_module", {}).get("ASR")
    new_asr_module = new_config.get("selected_module", {}).get("ASR")
    ...
    # 获取模块的具体配置
    current_config = before_config.get("ASR", {}).get(current_asr_module, {})
    new_config_val = new_config.get("ASR", {}).get(new_asr_module, {})
...
def is_valid_image_file(file_data: bytes) -> bool:
    ...
    # 容错：若前64字节内出现 JPEG SOI，则视为有效（处理偶发前导噪声/填充）
    try:
        idx = file_data.find(b"\xff\xd8\xff", 0, 64)
        if idx > 0:
            return True
    except Exception:
        pass
