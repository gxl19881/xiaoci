...
class MCPClient:
    ...
    def __init__(self):
        ...
        self.next_id = 10  # Start from 10 to avoid collision with handshake IDs (1=init, 2=list)
...
async def handle_mcp_message(conn, mcp_client: MCPClient, payload: dict):
    ...
    if "result" in payload:
        ...
        if msg_id in mcp_client.call_results:
            ...
            # 检查是否有 "System is busy" 错误，若有则播报提示
            if isinstance(result, dict) and result.get("success") is False:
                msg = result.get("message", "")
                if "System is busy" in msg:
                    if hasattr(conn, "tts"):
                        from core.providers.tts.dto.dto import ContentType
                        busy_text = "摄像头正忙，请稍作等待，系统将在10秒内自动重试。"
                        conn.tts.tts_one_sentence(conn, ContentType.TEXT, content_detail=busy_text)
            
            # 针对连拍逻辑增加特殊处理：
            # 如果是连拍第一张的响应（即 count > 1 且 result 包含特定指令），
            # 需要将其中的提示语提取并播报，而不是只作为工具执行结果返回给 LLM
            try:
                content = result.get("content", [])
                if isinstance(content, list) and len(content) > 0:
                    text_blob = str(content[0].get("text", ""))
                    # 检查是否包含“第一张照片已经拍摄完毕”等关键词
                    if "第一张照片已经拍摄完毕" in text_blob or "You MUST now say" in text_blob:
                        # 尝试从 JSON-in-String 中提取 message
                        try:
                            # 提取最内层 JSON
                            inner_json_str = text_blob.strip()
                            # 兼容可能包裹在 """ 或 ''' 中的情况
                            if inner_json_str.startswith('"') and inner_json_str.endswith('"'):
                                inner_json_str = json.loads(inner_json_str) 
                            
                            if isinstance(inner_json_str, str):
                                data = json.loads(inner_json_str)
                                if isinstance(data, dict):
                                    msg = data.get("message", "")
                                    # 提取 You MUST now say 之后的内容
                                    import re
                                    # [Modified] 更宽容的正则，适配不同的引号和 possible newlines
                                    match = re.search(r"say.*?: ['\"](.*?)['\"]", msg, re.DOTALL | re.IGNORECASE)
                                    if match:
                                        prompt_text = match.group(1)
                                        logger.bind(tag=TAG).info(f"拦截到中间交互指令: {prompt_text}")
                                        # 调用 TTS 播报提示并模拟为 LLM 的回复写入对话
                                        if hasattr(conn, "tts"):
                                            from core.providers.tts.dto.dto import ContentType
                                            conn.tts.tts_one_sentence(conn, ContentType.TEXT, content_detail=prompt_text)
                        except Exception as e:
                            logger.bind(tag=TAG).warning(f"解析中间结果失败: {e}")
                            pass
            except Exception:
                pass
...
async def call_mcp_tool(
    conn, mcp_client: MCPClient, tool_name: str, args: str = "{}", timeout: int = 30, skip_check: bool = False
):
    """
    调用指定的工具，并等待响应
    """
    if not skip_check and not await mcp_client.is_ready():
        raise RuntimeError("MCP客户端尚未准备就绪")

    if not skip_check and not mcp_client.has_tool(tool_name):
    ...
    # 记录请求ID与超时设置，便于问题定位
    logger.bind(tag=TAG).debug(
        f"MCP工具调用请求已发出: id={tool_call_id}, timeout={timeout}s"
    )
    try:
        await send_mcp_message(conn, payload)
    except Exception as e:
        # 发送失败需要清理本次future，避免后续悬挂导致永远等待
        await mcp_client.cleanup_call_result(tool_call_id)
        logger.bind(tag=TAG).error(
            f"发送MCP工具调用消息失败: id={tool_call_id}, tool={actual_name}, 错误={e}"
        )
        raise
    ...
    try:
        ...
        await mcp_client.cleanup_call_result(tool_call_id)
        raise TimeoutError(
            f"工具调用请求超时（id={tool_call_id}, tool={actual_name}, 等待{timeout}s）"
        )
    except Exception as e:
        await mcp_client.cleanup_call_result(tool_call_id)
        raise e
