...
public:
    ...
    // 发送格式控制：当为true时，上行发送原始PCM字节；否则发送Opus编码数据
    void SetSendFormatPcm(bool enable) { send_pcm_ = enable; }
    bool IsSendFormatPcm() const { return send_pcm_; }
...
private:
    ...
    bool send_pcm_ = true; // 缺省按照hello中约定发送PCM
