...
class ConnectionHandler:
    def __init__(
        ...
        # 输入与下行播放分离：
        # - audio_format: 记录客户端上行音频格式（来自 Hello），默认按 pcm 录入
        # - playback_audio_format: 服务端下行播放格式，默认强制 opus，可被环境变量覆盖
        self.audio_format = os.getenv("AUDIO_INPUT_FORMAT", "pcm").lower()
        self.playback_audio_format = (
            os.getenv("AUDIO_OUTPUT_FORMAT", "opus").lower()
        )
        if self.playback_audio_format not in ("opus", "pcm"):
            self.playback_audio_format = "opus"
    ...
    async def handle_connection(self, ws):
        try:
            ...
            try:
                self.logger.bind(tag=TAG).info(
                    f"下行播放编码: {self.playback_audio_format}"
                )
            except Exception:
                pass
    ...
    def _speak_vision_response(self, text: str) -> None:
        """分段播报视觉结果，确保完整的 FIRST->MIDDLE->LAST 序列。

        - 停止视觉保活循环（通过标记 _vision_final_sent）
        - 刷新活动时间，避免被动超时
        - 以新的 sentence_id 播报该条结果，避免与现有会话串流冲突
        - 将文本追加进对话历史，便于记忆/上报
        """
        try:
            if text is None:
            ...
            if hasattr(self, "tts") and self.tts is not None:
                ...
                # 正文分段播报
                self.tts.tts_one_sentence(self, ContentType.TEXT, content_detail=msg, sentence_id=sid)
                # 结尾 LAST，确保播放器正确收尾
                self.tts.tts_text_queue.put(
                    TTSMessageDTO(
                        sentence_id=sid,
            ...
            # 追加到对话历史，便于后续记忆/追踪
            try:
                self.dialogue.put(Message(role="assistant", content=msg))
            except Exception:
                pass
            ...
            try:
                self.logger.bind(tag=TAG).info(
                    f"视觉结果播报完成，长度={len(msg)}"
                )
            except Exception:
                pass
        except Exception as e:
            try:
                self.logger.bind(tag=TAG).warning(f"_speak_vision_response 执行失败: {e}")
            except Exception:
                pass
